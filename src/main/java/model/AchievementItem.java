...
public class AchievementItem implements Achievement{
    ...
    public int calcPoints(Player player) { //point = base * min(count_simboli)
        ...
        int min_val = 0;
        if (this.basePoint == 3){
            if (player.getSymbolCount().get(symbol.get(0)) <= player.getSymbolCount().get(symbol.get(1))){
                min_val = player.getSymbolCount().get(symbol.getFirst());
            }
            else{
                min_val = player.getSymbolCount().get(symbol.get(1));
            }
            if (min_val > player.getSymbolCount().get(symbol.get(2))){
                min_val = player.getSymbolCount().get(symbol.get(2));
            }
            point = this.basePoint * min_val;
        } else if (this.basePoint == 2) {
            point = this.basePoint * (Math.floorDiv(player.getSymbolCount().get(symbol.getFirst()), 2));
        }
        return point;
    }
}
